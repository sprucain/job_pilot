...
Last updated: 2026-09-16 (Generate Resume from Profile)
...
**Generate Resume from Profile** (ResumeUpload.tsx / ProfileForm.tsx, new in Feature 08, 2026-09-16; hardened in the same-day `/review` follow-up): the button at the bottom of the Resume card (previously a static stub) is wired to `onGenerate`/`isGenerating`/`generateError` props — same loading/disabled/error shape as Extract from Resume above (`disabled:cursor-not-allowed disabled:opacity-60`, label swaps to "Generating...", a `text-sm text-error` line below the row on failure). No new visual pattern — reuses the existing primary button + inline error convention. `ProfileForm.tsx`'s `handleGenerate` always runs the same save path as the Save Profile button first (`persistProfileCore()`, factored out of the old inline `handleSave` body so both callers share it — see the `/review` note below for why it no longer touches `saveState` itself), then calls `/api/resume/generate` — so the generated PDF always reflects what's currently on screen, never a stale DB snapshot. Since Generate overwrites the single `resumes/{user_id}/resume.pdf` slot — including a manually-uploaded original, which has no other copy anywhere — it now gates behind a native `window.confirm()` first (only when a resume already exists to lose), the same guard Delete already used.

**Cross-action busy lock + generated-resume Extract guard** (`/review` follow-up, 2026-09-16): Save/Delete/Extract/Generate are four independent `useTransition`s that all read or write the same resume file and/or `profiles` row — `ProfileForm.tsx` now computes one `isBusy = isPending || isDeletingResume || isExtracting || isGenerating` and passes it to `ResumeUpload` as a new `busy` prop; every mutating control there (Replace, Remove, Extract, Generate, the dropzone, and its hidden file input) disables on `busy` in addition to its own specific pending flag, and the Save Profile button in `ProfileForm.tsx` does the same. This is what actually prevents the concrete races (e.g. clicking Remove while a Generate upload is still in flight, which could resurrect a resume the user just deleted) — reuse this `isBusy`-computed-once-passed-down-as-one-prop shape for any future action added to this card rather than adding a fifth independent transition with no cross-lock. Separately, a new `isGeneratedResume` boolean (true after a successful Generate, cleared the moment the user picks their own file or deletes) disables the Extract button with swapped copy ("This resume was generated from your profile — upload your own resume to use Extract.") — extracting from a document GLM just wrote *from this same profile* would just feed its own output back into the form.
...
`lib/resume-generation.ts` (Feature 08, Venice call + lenient JSON normalization, same shape as `lib/resume-extraction.ts`) and `lib/resume-pdf.tsx` (Feature 08, the `@react-pdf/renderer` `Document`/`Page` template + `renderToBuffer` wrapper) are deliberately separate files — the `.tsx` extension is isolated to the one file that actually needs JSX for the PDF template, keeping the Venice/content logic in a plain `.ts` module. Neither lives in `agent/` — same reasoning as Feature 07's `lib/resume-extraction.ts`: synchronous, single-shot, no `run_id`/`job_id`, no `agent_logs` entry. `app/api/resume/generate/route.ts` reads the profile straight from the DB via `lib/profile-server.ts`'s `getCurrentUserProfile()` (not from the client) and requires `fullName` + at least one work-experience entry with a non-blank title/company before calling Venice, returning a 422 with a user-facing message otherwise — fully-blank entries are filtered out of what's actually sent to Venice/rendered into the PDF, not just gated at the validation check.

**Shared server-side helpers, added in the Feature 08 `/review` follow-up (2026-09-16)** — extracted out of code that had drifted into 2-3 near-identical copies (flagged across several independent review passes):
- `lib/profile-server.ts`'s `getCurrentUserProfile(insforge)` — the "auth.getCurrentUser → fetch profiles row → mapProfileRowToUi" sequence, now shared by `app/profile/page.tsx` and `app/api/resume/generate/route.ts`. `actions/profile.ts`'s `saveProfile` deliberately does NOT use this — it only ever needs 2 narrow columns to decide insert-vs-update, not a full mapped `Profile`.
- `lib/resume-storage.ts`'s `getResumeStorageKey(userId)` / `uploadResumeFile(insforge, userId, file)` / `signResumePreviewUrl(insforge, resumePdfUrl)` — the upload-to-the-fixed-key and best-effort-sign steps, shared by `actions/profile.ts`'s manual-upload path and the generate route's AI-written path. Each caller still owns its own DB `update` call (whole-profile update vs. single-field update — genuinely different semantics, not worth forcing into the shared function).
- `lib/venice-json.ts`'s `callVeniceJson(options)` — the "call Venice with JSON-mode + `disable_thinking` → parse" scaffold, shared by `lib/resume-extraction.ts` and `lib/resume-generation.ts`. Also the one place that checks `finish_reason === "length"` and reports a distinct `"truncated"` reason, so a token-budget cutoff (silently possible at `resume-generation.ts`'s 1000-token budget with 3 lengthy roles) doesn't collapse into the same generic "please try again" a real parse failure gets — retrying identical input after a truncation fails identically, so callers map `"truncated"` to different, more actionable copy.
- `lib/json-normalize.ts`'s `asString`/`asNumber`/`asStringArray` — the runtime-narrowing coercions for lenient GLM JSON parsing, shared by both files above.

Each caller still owns its own user-facing error strings (`RESUME_GENERATION_FAILED_ERROR` in `lib/constants.ts`, `EXTRACTION_FAILED_ERROR` local to `lib/resume-extraction.ts`) — the shared helpers return typed failure reasons, never hardcoded copy, so extraction and generation can keep their own wording.

**Positional-mismatch guard in `lib/resume-generation.ts`** (`/review` follow-up, 2026-09-16): the original version zipped GLM's `workExperience` bullets onto `profile.workExperience` by array index alone, trusting the prompt's "same order" instruction with no verification — if GLM ever reordered roles (e.g. defaulting to reverse-chronological), bullets could get silently attributed to the wrong role. Fixed by having the prompt ask GLM to echo back each role's exact `title`/`company` alongside its bullets; `normalizeGeneratedContent` now verifies that echo matches `profile.workExperience[index]` (case/whitespace-insensitive) before trusting the bullets, and falls back to a naive sentence-split of that role's own raw `responsibilities` text when it doesn't match (or GLM omitted the role) — so a role always renders with *something* rather than either nothing or someone else's bullets.
