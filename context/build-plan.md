...
- Session management
- `proxy.ts` (Next.js 16's renamed `middleware.ts`) protecting /dashboard, /profile, /find-jobs, /find-jobs/[id]
...
- Create `instrumentation-client.ts` at the project root — PostHog browser client, initialized with `NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN` and `NEXT_PUBLIC_POSTHOG_HOST` (Next.js 15.3+ file convention, auto-loaded — never manually imported into `app/layout.tsx`, and never combined with a `PostHogProvider`)
- Create lib/posthog-server.ts — PostHog server client (`getPostHogClient()`, singleton) with flushAt: 1 and flushInterval: 0
- `next.config.ts` reverse-proxy rewrites (`/ingest/*` → PostHog's ingestion/asset hosts) so client-side capture isn't blocked by ad blockers
- posthog.identify() called after successful login with user ID
- posthog.reset() called on logout — requires a client-side trigger; not wired yet since no sign-out UI exists (see progress-tracker.md)
...
Extract from Resume button — GLM 5.2 reads uploaded PDF and auto-fills profile form fields.
...
- GLM 5.2 reads extracted text and returns structured JSON matching all profile field names
...
Generate a clean professional PDF resume from current profile data using GLM 5.2.

**Logic:**

- POST /api/resume/generate
- Reads current profile data from profiles table
- GLM 5.2 generates professional resume content:
  - Professional summary paragraph
  - Polished work experience bullet points
  - Clean professional language throughout
- @react-pdf/renderer renders GLM 5.2 output into clean single-page PDF using renderToBuffer()
...
- For each job returned:
  ...
  - GLM 5.2 scores job against user profile:
...
- AI Match Reasoning section — match reason paragraph from GLM 5.2
...
- Close Browserbase session after homepage + max 3 sub-pages
  **GLM 5.2 synthesis (runs after browser closes):**
...
- Merge and sort all by started_at (agent_runs) / found_at (jobs) descending — take last 5-10 entries
