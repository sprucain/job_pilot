...
│   │   │   └── page.tsx                   → Login page
│   │   └── callback/
│   │       └── route.ts                   → OAuth callback handler (Route Handler — SSR mode does not auto-exchange in the browser)
│   ├── dashboard/
...
- Public routes: /, /login
- `proxy.ts` (Next.js 16 renamed `middleware.ts` → `proxy.ts`) does an optimistic cookie-presence check on every protected route via `updateSession()` from `@insforge/sdk/ssr/middleware`; real `getCurrentUser()` checks happen in each protected Server Component
...
Note: `@insforge/ssr` is not a real package — the SSR helpers are subpath exports of `@insforge/sdk` (`@insforge/sdk/ssr`, `@insforge/sdk/ssr/middleware`). Both `createBrowserClient()`/`createServerClient()` read config from `NEXT_PUBLIC_INSFORGE_URL`/`NEXT_PUBLIC_INSFORGE_ANON_KEY` automatically — no need to pass them explicitly.
