...
| `login_initiated`    | User clicks a Google/GitHub sign-in button | provider                    |
| `login_failed`       | OAuth code exchange fails in the callback  | reason                      |
| `login_completed`    | OAuth callback succeeds, session created   | (identified by userId)      |

These seven events are the only events in this project. Do not add more without updating this list first. The three `login_*` events are server-side only (fired from `actions/auth.ts` / `app/(auth)/callback/route.ts`) since the OAuth redirect flow isn't observable client-side.

Pre-login `distinctId` is a random UUID generated in `signInWithGoogle`/`signInWithGithub`, persisted in a short-lived cookie (`POSTHOG_LOGIN_DISTINCT_ID_COOKIE`) so `login_failed`/`login_completed` can reuse the exact same id `login_initiated` used — never hardcode `"anonymous"` as a literal distinctId, that merges every anonymous visitor's events into one fake PostHog person. On successful login, `posthog.alias({ distinctId: <that id>, alias: user.id })` links the pre-login attempt to the now-known person before `identify()`/the `login_completed` capture.

Every `posthog.flush()` call goes through `flushPostHogSafely()` in `lib/posthog-server.ts`, never a bare `await client.flush()` — PostHog's own SDK docs flush() as throwing on network/HTTP errors, and an analytics outage must never be able to break the request it's instrumenting (e.g. a successful login failing to redirect because PostHog was unreachable).
